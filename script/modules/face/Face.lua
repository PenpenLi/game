...
local ns = 'face'
FaceList = FaceList or {}
Account2Rank = Account2Rank or {}
UpRank = UpRank or {}
UserFaceList = UserFaceList or {}
...
function loadDB()
	...
	while(true) do
		...
		--table.insert(FaceList,tmp)
		FaceList[tmp.rank] = tmp
		...
		UserFaceList[tmp.account] = UserFaceList[tmp.account] or {}
		table.insert(UserFaceList[tmp.account],tmp.rank)
		UserFaceList[tmp.accountBy] = UserFaceList[tmp.accountBy] or {}
		table.insert(UserFaceList[tmp.accountBy],tmp.rank)
	end
	--table.sort(FaceList,function(a,b)return a.rank < b.rank end)
	--print("Arena:loadDB::"..(_USec()-begin))
end

function save(isSync)
	for i=1,#UpRank do
		local data = FaceList[UpRank[i]]
...
function addRank(human,id)
	if ApplyList[human.db.account] == nil and UserFaceList[human.db.account] then 
		if #UserFaceList[human.db.account] + 1 > Config[1].numMax then
			return false
		end
	end 

	if UserFaceList[human.db.account] == nil and ApplyList[human.db.account] then 
		if #ApplyList[human.db.account] + 1 > Config[1].numMax then
			return false
		end
	end 

	if UserFaceList[human.db.account] and ApplyList[human.db.account] then 
		if #UserFaceList[human.db.account] + 1 + #ApplyList[human.db.account] > Config[1].numMax then
			return false
		end
	end 
...
function getFaceList(human)
	local List = {}
	local tmp = UserFaceList[human.db.account]
	if tmp == nil then 
		return
	end

	for i=1,#tmp do
		if FaceList[tmp[i]] then 
			local data = {}
			local temp = {}
			if  FaceList[tmp[i]].account == human.db.account then 
				local returnHuman = HumanManager.getOnline(FaceList[tmp[i]].accountBy) or HumanManager.loadOffline(FaceList[tmp[i]].accountBy)
				data.id = FaceList[tmp[i]].accountBy
				data.name = returnHuman.db.name
				data.lv = returnHuman.db.lv
				data.fighting = Arena.getArenaFightVal(FaceList[tmp[i]].accountBy)
				data.icon = returnHuman.db.bodyId
				data.arena = ArenaLogic.makeByIdEnemyData(FaceList[tmp[i]].accountBy)

				if HumanManager.getOnline(FaceList[tmp[i]].accountBy) == nil then 
					data.isOnline = 0
				else
					data.isOnline = 1
		   		 end
				...
				local returnHuman = HumanManager.getOnline(FaceList[tmp[i]].account) or HumanManager.loadOffline(FaceList[tmp[i]].account)
				data.id = FaceList[tmp[i]].account
				data.name = returnHuman.db.name
				data.lv = returnHuman.db.lv
				data.fighting = Arena.getArenaFightVal(FaceList[tmp[i]].account)
				data.icon = returnHuman.db.bodyId
				data.arena = ArenaLogic.makeByIdEnemyData(FaceList[tmp[i]].account)
				
				if HumanManager.getOnline(FaceList[tmp[i]].account) == nil then 
					data.isOnline = 0
				else
					data.isOnline = 1
		 	   end
...
function checkIsFace(accountA,accountB)
...
function delRank(human,targetAccount)
	...
	if FaceList[rank] then 
		FaceList[rank] = nil
	end 

	if UserFaceList[account] ~= nil then 
		for i,v in pairs(UserFaceList[account]) do
			if v == rank then 
				table.remove(UserFaceList[account],i)
			end
		end
	end
	
	if UserFaceList[targetAccount] ~= nil then 
		for i,v in pairs(UserFaceList[targetAccount]) do
			if v == rank then 
				table.remove(UserFaceList[targetAccount],i)
			end
		end
	end
...
function updateRank(human,account)
	...
	--local ret = false
	--if FaceList[rank] == nil then 
	--	return false
	--end
	index = index +1
	FaceList[index] = newFaceRankDB(human,account,index)
	local rank = index
	Account2Rank[human.db.account.."#".. account] = rank
	Account2Rank[account.."#"..human.db.account] = rank

	table.insert(UpRank,rank) 
	UserFaceList[human.db.account] = UserFaceList[human.db.account] or {}
	table.insert(UserFaceList[human.db.account],rank)
	UserFaceList[account] = UserFaceList[account] or {}
	table.insert(UserFaceList[account],rank)
	...
	return ret
	--table.insert(human.db.FaceDB,rank)
end

function newFaceRankDB(human,id,rank)
