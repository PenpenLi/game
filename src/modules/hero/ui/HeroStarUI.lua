...
function new(name,star)
	-- star 2--5
	local ctrl = Control.new(require("res/hero/HeroStarSkin"),{"res/hero/HeroStar.plist","res/common/an.plist"})
...
function init(self,name,star)
	...
	if star == 2 then
		self.txtsm:setVisible(true)
		self.txtsm:setString('开启"二星大招"特效')
	elseif star == 3 then
		self.txtsm:setVisible(true)
		self.txtsm:setString('开启"三星破招"特效')
	elseif star == 4 then
		self.txtsm:setVisible(true)
		self.txtsm:setString('开启"四星续招"特效')
	elseif star == 5 then
		self.txtsm:setVisible(true)
		self.txtsm:setString('开启"五星专精"特效')
	else
		self.txtsm:setVisible(false)
	end

	AudioEngine.playEffect("res/sound/fight/common/Success.mp3",false)

	local function onBack(self,event,target)
		UIManager.removeUI(self)
	end
	self.confirm:addEventListener(Event.Click,onBack,self)
...
function getStandPosition(self)
	local size = self.confirm:getContentSize()
	local px,py = self.confirm:getPosition()
	-- return (px+size.width/2)*Stage.uiScale,(py + 50)*Stage.uiScale
	return px+size.width/2,py+100
end
