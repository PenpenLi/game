...
function setSkin(self)
	...
	self:addSpriteFrames("res/armature/athena/AthenaSkin.plist")
	Hero.setSkin(self,boneRes)
end

function hit_1713(self,bone,evt,originFrameIndex,currentFrameIndex)
...
function doAfterPlay(self,isHarm)
	...
	if self.curState.name == 1710 then
...
function startAssist(self)
	...
	self:addEventListener(Event.PlayEnd,function(self,event) 
		if event.stateName == "jump" then
			self:play(1729,true)
		elseif event.stateName == 1729 then
