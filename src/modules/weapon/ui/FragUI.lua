...
function init(self, itemId, maxCnt)
	_M.touch = Common.outSideTouch
	CommonGrid.bind(self.frag.iconbg)
	self.frag.iconbg:setItemIcon(itemId)
	local itemCfg = ItemConfig[itemId]
	self.frag.txtheroname:setString(itemCfg.name)
	local fragmentNum = BagData.getItemNumByItemId(itemId)
	self.frag.progress.txtprogress:setString(fragmentNum.."/"..maxCnt)
			...
			if target.url == nil and self.txtnotopened:isVisible() then
				...
				if target.url then
					if target.lv and Master.getInstance().lv < target.lv then
						...
						local ui = UIManager.replaceUI(target.url)
						ui:setPositionY(0)
					end
				else
	...
	local levelList = Chapter.getLevelListByReward(itemId)
	local itemList = {}
	...
	local shopList = Shop.getShopListById(itemId)
	if #shopList > 0 then
