...
function getHero_maxHp(value,lv,inFactor,qualityFactor)
...
function getHero_attr(value,lv,inFactor,qualityFactor)
	return math.floor(lv*lv*attr_args[1]+lv*attr_args[2]+attr_args[3])
	-- return math.floor((lv*lv*0.2+0.8*lv+4)*inFactor*qualityFactor)
end

getHero_atkA = getHero_attr
getHero_defA = getHero_attr
getHero_atkB = getHero_attr
getHero_defB = getHero_attr
getHero_atkC = getHero_attr
getHero_defC = getHero_attr



--int(round(等级*等级*COST参数1+等级*COST参数2+COST参数3,0))
function getHero_cost(value,lv,inFactor,qualityFactor)
	return math.floor(lv*lv*cost_args[1]+lv*cost_args[2]+cost_args[3])
	-- if lv == 1 then
...
function getHero_hpR(value,lv,inFactor,qualityFactor)
	if value then
		return value
	else
		return 1
...
function getHero_rageR(value,lv,inFactor,qualityFactor)
	if value then
		return value
	else
		return 1
...
function getHero_assist(value,lv,inFactor,qualityFactor)
	if value then
		return value
	else
		return 1
