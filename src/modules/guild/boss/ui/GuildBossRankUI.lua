...
function refreshList(self)
	...
	local dataList = GuildBossData:getRankList()
	if #dataList ~= 0 then
		self.noRankTipTxt:setVisible(false)

		local function onCheckTeam(rank, evt)
			Network.sendMsg(PacketID.CG_GUILD_BOSS_CHECK_TEAM, rank)
		end

		local function refreshItem(item, data)
			CommonGrid.bind(item.headBG)
			item.headBG:setBodyIcon(data.icon)
...
function showTeamUI(self, rank, fighting, flowerCount, heroList)
	local data = GuildBossData:getRankList()[rank]
